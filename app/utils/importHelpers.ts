...
export const fetchFromUrl = async (url: string, format: string = 'text'): Promise<{ content: string; metadata?: Record<string, any> }> => {
  try {
...
export function scheduleUrlCheck(
  ...
  const checkInterval = setInterval(async () => {
    try {
      ...
      console.error('Error checking URL for updates:', error)
    }
